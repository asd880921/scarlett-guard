...
### 常駐
- **單一實例** —— 重複啟動不會開出第二份，而是把已在執行的視窗叫出來。
  這程式會註冊全域熱鍵並重置 PnP 裝置，跑兩份只會有兩個系統匣圖示、
  兩個熱鍵監聽器搶同一組按鍵，以及兩邊同時對裝置下重置指令的風險
- 系統匣圖示帶狀態色（正常／忙碌／異常）
- 關閉視窗收進系統匣，熱鍵持續有效
...
裝置狀態必須透過 PowerShell 查詢，約需 2–3 秒；其餘資料（設定、紀錄、統計、
路徑、輸入裝置清單）都在記憶體裡、瞬間可得。所以 `bootstrap()` **刻意不包含
裝置狀態** —— 否則整個 UI 會陪著它一起空白好幾秒。前端先把快的畫上去，
再非同步去要裝置狀態，期間狀態圓點以呼吸動畫表示「還在查」而不是「查不到」。

裝置查詢本身也從三次 PowerShell 呼叫（裝置清單、主要裝置、驅動屬性）
合併成單一腳本 —— 成本幾乎全在 PowerShell 的行程啟動，不在查詢本身。
實測 5.8 秒降到 2.7 秒。
...
src/scarlett_guard/
  ...
  elevation.py             UAC 提權
  single_instance.py       單一實例（具名 Mutex + 喚醒 Event）
